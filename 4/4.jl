...
# ╔═╡ 85f83eaa-5dbd-4a6f-a33a-1e51009a82f5
begin
	using WGLMakie
	using Bonito
	using Makie
...
# ╔═╡ 2fe25067-7c8f-47e5-a66d-878a90c75806
begin
	v = reshape(read("input"), 141, 140)[1:140, :];
	xmas = repeat(b"XMAS",1);
	xm = tuple(xmas[1:4]...) :: Tuple{UInt8, UInt8, UInt8, UInt8}
	isXmSa = (x) -> (x == xm) || (x == reverse(xm))
	rot45Sq = (M) ->
	  [1:2*size(M)[1]] ⨟ Map((x) -> [M[x-i+1, i] for i = 1..x]) ⨟ collect;
	#lastindex(rot45Sq(v)[1])
end

# ╔═╡ 1750bc47-57a4-4967-a55a-11623d5d188e
begin
	count = 0
	# xmas = repeat(b"XMAS",35,280)
	for i in axes(v, 1)
	  for j in axes(v, 2)
		if (i > 3)
		    view = tuple(v[i-3:i,j]...)
    		if isXmSa(view)
		    	count += 1
	    	end
		...
		if (j > 3)
		    view = tuple(v[i,j-3:j]...)
    		if isXmSa(view)
		    	count += 1
	    	end
		end
		if ((i>3) && (j>3))
			view = (v[i,j], v[i-1,j-1], v[i-2,j-2], v[i-3,j-3])
			if isXmSa(view)
		    	count += 1
	    	end
		end
	...
	count
	# horizontal counts
	# v -> rot90(_, i) -> _[j..140-4+j, :] -> reshape(_,4,:) ->
	#   -> & xmas[k] along first axis
	# foldl(&, reshape(rot90(v,i),4,:)
	# reshape(v,4,:)[i,:] .== xmas[i]
	# println(hcat(v, rotr90(v)) ⨟ eachcol ⨟ Map(Consecutive(4; step = 1)) ⨟ Map(Filter(isXmSa)) ⨟ Map(length) ⨟ foldxl(+))
	# vertical counts
	
	# diagonal counts
end

# ╔═╡ afc5d311-1391-4c98-aea4-6901e15f4d53
length(filter( (x) -> x == repeat(b"XMAS",1), reshape(v, 4, :)))

# ╔═╡ 41dd95e8-e217-4e10-a5b9-f12941bdd10c
begin 
	A,M,S = 0x41,0x4d,0x53
	cor=[[M,M,S,S] [M,S,M,S] [S,M,S,M] [S,S,M,M]] :: Array{UInt8}
	dirs=[CartesianIndex(-1, -1),CartesianIndex(1, -1),CartesianIndex(-1, 1),CartesianIndex(1, 1)]
	cor=map((x) -> collect(zip(x,dirs)), eachcol(cor))
	dump(cor)
	cb = (I) -> checkbounds(Bool, v, I)
	ok = (I) -> cb(I - CartesianIndex(1, 1)) && cb(I + CartesianIndex(1, 1))
	count2 = 0
	for I in CartesianIndices(v)
	   if (ok(I) && v[I]==A)
		   w = filter((col) -> all(map(((c,i),) -> v[I+i]==c, col)), cor);
		   (count2<10) ? dump(w) : nothing
		   count2 += length(w)
	   end
	end
    count2
...
# ╔═╡ c53cc105-e872-4d25-8314-ff02c7c48e34


# ╔═╡ 630647de-4935-41bd-afbd-3e15d9d8aaf4
# ╠═╡ disabled = true
#=╠═╡
begin
	...
	ax = Axis(f[1,1])
	hidedecorations!(ax)
	heatmap!(ax, v, colormap=Reverse(:ice))
	f
end
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "d2821c0abe26d1e013b9d97a373a8685ac5967bb"
